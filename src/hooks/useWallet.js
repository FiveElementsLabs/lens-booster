...
export const useWallet = () => {
  ...
  const networkChanged = async () => {
    ...
    dispatch({ type: actions.CHANGE_NETWORK, payload: { network_name, chain_id, provider: Web3Provider } });
  };
  ...
  const changeNetwork = async (networkName) => {
    ...
    try {
      await window.ethereum.request({ method: 'wallet_addEthereumChain', params: [{ ...networks[networkName] }] });

      const { name: network_name, chainId: chain_id } = await provider.getNetwork();
      dispatch({ type: actions.CHANGE_NETWORK, payload: { network_name, chain_id } });
    } catch (err) {
